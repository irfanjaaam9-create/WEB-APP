  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {machineryCategories.length > 0 && <div className="mb-12 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-center"><h2 className="mb-4 text-sm font-black uppercase tracking-wider text-slate-900">Machinery Categories</h2><div className="flex flex-wrap gap-3 justify-center">{machineryCategories.map((category: any) => <Link key={category.id} href={`/machinery/${category.slug}`} className="rounded-full border border-amber-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-amber-500 hover:text-amber-700">{category.name}</Link>)}</div></div>}
